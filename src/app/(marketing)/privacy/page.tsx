...
import { PRIVACY_MARKDOWN, LEGAL_LAST_UPDATED } from "@/lib/marketing/legal";
...
export async function generateMetadata(): Promise<Metadata> {
  ...
  return { title: `${t("marketing.legal.privacyTitle")} — ${BRAND_NAME}` };
}

export default async function PrivacyPage() {
  const { t, locale } = await getMarketingTranslator();
  const html = renderMarkdown(PRIVACY_MARKDOWN[locale]);

  return (
    <div className="mx-auto max-w-3xl px-4 py-16">
      <h1 className="text-3xl font-bold tracking-tight">{t("marketing.legal.privacyTitle")}</h1>
