...
| **6 — Ecosystem (lowest priority)** | Third-party extensibility | Public API, ~~webhooks~~, developer OAuth app-install flow, revenue-shared app marketplace — deliberately last, since it only matters once you have a real merchant base for developers to build for. **Webhooks cut from the active roadmap 2026-09-01 (user decision)** — already built and shipped (signed outbound event push, `order.created`/`order.status_changed`/`order.paid`); the code stays in the repo as-is, but no further work is planned on it without asking. |
